...
class AdminController extends Controller
{
    ...
    public function getAdminIndex(){
        ...
        $jobs = JobApplications::all()->count();
        ...
        return view('admin.dashboard', compact('messages','jobs','service','request','requests'));
    }
    ...
    public function userManagement(){
        ...
        $user = User::where('Role','=', "Artisan")->orderBy('id','DESC')->simplePaginate(20);
    ...
    public function getMergingIndex(Request $request){

        if(!empty($request->segment(5))){
            $jobs = EmployerJobs::orderBy('id','DESC')->where('id', $request->segment(5))->get();
        }else {
            $jobs = EmployerJobs::orderBy('id','DESC')->get();
        }

        foreach ($jobs as $row){
            $row->full_name  = User::where('id', $row->user_id)->value('full_name');
        }

        $candidate = User::where('role',"Artisan")->where('status',1) ->get();
        return view('admin/merging', compact('jobs','candidate'));
    }

    public function saveMergingRequest(Request $request){
        ...
        if($check > 0){
         return back()->withInput()->with('response', "Job can't be Merged the second time.");
        }
